...
const struct coordinate RIGHT = { 1, 0 };//navigations from taking origin from top left corner//
...
enum { WALL, NOTHING } game_area[GAME_AREA_WIDTH][GAME_AREA_HEIGHT];
...
main ()
{
  screen_init ();
  curs_set (0);
  mvprintw (0, 10, "GAURAV MAZE GAME");
  while (1)
    {
      generate_game_area ();
      draw_game_area ();
